  ...
  return {
    ...
    details:
      typeof selectedReservation.details === "string"
        ? safeJsonParse(selectedReservation.details)
...
export async function updateReservation({
  ...
  return await db
    .update(reservation)
    .set({ hasCompletedPayment })
    .where(eq(reservation.id, id));
}

// ✅ NEW: Get all reservations for a user (used by listTickets tool)
export async function getReservationsByUserId({
  ...
  try {
    ...
    console.error("Failed to get reservations by user from database");
    throw error;
  }
}
